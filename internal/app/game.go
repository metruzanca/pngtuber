// Top-level Ebitengine game logic. Milestones 1-4 wired the transparent
...
// Game is the top-level Ebitengine game.
type Game struct {
	cfg          *Config
	char         *Character
	input        input.Backend
...
// NewGame builds the game from the resolved assets, connecting the input
// backend and mic, and wiring activity transitions to the character.
...
// Close releases the global input backend and microphone.
func (g *Game) Close() {
	g.input.Close()
	if g.mic != nil {
		g.mic.Close()
